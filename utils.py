...
# pseudo labels                                      
def obtain_label(loader, netE, classifiers, args, c=None):
    start_test = True
    netE.eval()
    ...
    with torch.no_grad():
        ...
        for i in range(len(loader)):
            ...
            inputs = inputs.cuda()
            feas = netE(inputs)
            outputs_list = [f(feas) for f in classifiers]
...
def gradient_discrepancy_loss_margin(args, preds_s, s_y, preds_t, t_y, netE, classifiers):
    loss_w = Weighted_CrossEntropy
    loss = nn.CrossEntropyLoss()
    #CrossEntropyLabelSmooth(num_classes=args.class_num, epsilon=0.1)
    # gm loss
    ...
    for p_s, p_t, netC in zip (preds_s, preds_t, classifiers):

        src_loss = loss(p_s, s_y)
        tgt_loss = loss_w(p_t, t_y)
        ...
        for n, p in netC.named_parameters():
            ...
            if len(p.shape) > 1:
                ...
                _cossim = F.cosine_similarity(fake_grad, real_grad, dim=0)
            grad_cossim_list.append(_cossim)

        grad_cossim_stack = torch.stack(grad_cossim_list)
        gm_loss_i = (1.0 - grad_cossim_stack).mean()
        gm_loss_list.append(gm_loss_i)

    if not gm_loss_list:
        return torch.tensor(0.0, device = s_y.device)
    
    gm_loss = torch.stack(gm_loss_list).mean()
    return gm_loss
